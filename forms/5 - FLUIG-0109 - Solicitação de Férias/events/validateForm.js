function validateForm(form) {
	...
	if (atividade == 153 && acaoUsuario == "true") { // Verifica se é a atividade 93 e se o usuário está ENVIANDO a tarefa
		...
		log.info("Valor cpAnexoValidado: " + anexoValidado); // Log para depuração
		log.info("Valor cpFeriasValidada: " + feriasValidada); // Log para depuração

		// Valida se o checkbox "Anexo validado" foi marcado
		// Checkboxes marcados no Fluig retornam "on", desmarcados retornam null ou ""
		if (anexoValidado != "on") {
		...
		if (feriasValidada != "on") {
			// Ajustei o texto da mensagem para corresponder ao label do HTML
			Errors.push("É obrigatório marcar a opção 'Kit Férias validado (Anexos e Dados OK)' para prosseguir.");
		}

		// Validação da Avaliação (Mantida - verificar se ainda é necessária/correta)
		var aprovacaoRH = form.getValue("cpAprovarGestor3"); // Decisão do RH na etapa anterior (136/153)
		if (aprovacaoRH == '1') { // Só exige avaliação se o RH validou previamente
        ...
        log.info("--- Fim da validação da Atividade 93. Erros encontrados: " + Errors.length + " ---"); // Log para depuração
	}
    // ****** FIM DO BLOCO PARA ATIVIDADE 93 ******
