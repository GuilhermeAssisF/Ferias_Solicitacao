$(document).ready(function () {
	...
	// Lógica específica para a Atividade 90 (Gerar Kit Férias)
	if (atividade == 90) {
		// Garante que o checkbox Kit Férias comece desabilitado
		$("#cpFlagKitFerias").prop("disabled", true);
		// Limpa o campo oculto de visualização ao carregar
		$("#rhAnexoVisualizado").val("");

		// Define a ação do botão "Visualizar Anexos RH"
		$("#btnVisualizarAnexosRH").on("click", function () {
			// 1. Marca o campo oculto como visualizado
			$("#rhAnexoVisualizado").val("true");

			// 2. Habilita o checkbox "Kit Férias"
			$("#cpFlagKitFerias").prop("disabled", false);

			// 3. (Opcional) Tenta ir para a aba de anexos
			var anexoTab = parent.$('a[href="#attachments"]'); // Acessa elementos da página pai do Fluig
			if (anexoTab.length > 0) {
				...
				setTimeout(function () {
					try {
						...
						console.error("Erro ao tentar clicar na aba Anexos:", e);
						FLUIGC.toast({ title: 'Anexos:', message: 'Por favor, clique manualmente na aba "Anexos".', type: 'info' });
					}
				}, 150);
			} else {
				console.warn("Aba de anexos não encontrada via seletor 'a[href=\"#attachments\"]'.");
				FLUIGC.toast({ title: 'Anexos:', message: 'Por favor, clique manualmente na aba "Anexos".', type: 'info' });
			}

			// 4. (Opcional) Desabilita o próprio botão após o clique para evitar cliques múltiplos
			$(this).prop("disabled", true);
			$(this).text("Anexos Indicados para Visualização"); // Muda o texto do botão
		});
	}
